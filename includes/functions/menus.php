...
function menu_file_path($additionalMenu = ''){
	...
	if($additionalMenu != ''){
		$redesign_modifier = $additionalMenu;
	}
...
function quick_menu($before='', $after='', $additionalMenu = '', $custom_caret = ''){ //pulls domain from config file
	global $menu;
	$menu = '';
	$menu .= '<ul class="nav navbar-nav men-level-'.$ch_level.'">'.$before;
	$file = menu_file_path($additionalMenu);
	...
	$menu_id = 'nav_menu';
	if($additionalMenu != ''){
		$menu_id .= '_' . $additionalMenu;
	}
	$menu .= $after.'</ul>';
	echo '<div id="' . $menu_id . '" class="NP collapse navbar-collapse navbar-ex1-collapse">'.$menu.'</div>';
